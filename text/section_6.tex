...
The full implementation code can be accessed at \url{https://github.com/phamtranminhtri/music_rag}
...
\subsection{Python Flask app for the chatbot}

% \subsection{System Architecture}

The application is built as a web-based conversational agent specifically designed for music-related queries. The architecture leverages the Flask microframework for the backend, orchestrating interaction between the user interface, external APIs, and a Retrieval-Augmented Generation (RAG) pipeline. The system components are detailed below:

\subsubsection{Backend Core}
The backend is implemented in Python using Flask. It exposes several RESTful endpoints to handle user interactions:
\begin{itemize}
    \item \texttt{/} (GET): Serves the main web interface and initializes a unique session identifier for conversation history tracking.
    \item \texttt{/chat} (POST): The primary endpoint for processing text-based user queries. It invokes the LangChain agent to generate responses.
    \item \texttt{/stt} (POST): Handles Speech-to-Text conversion. It accepts audio files (WebM format), converts them to WAV using \textit{pydub}, and utilizes the \textit{SpeechRecognition} library with the Google Speech Recognition API to transcribe user speech.
    \item \texttt{/tts} (POST): Handles Text-to-Speech conversion. It detects the language of the agent's response using \textit{langdetect} and synthesizes audio using the Google Text-to-Speech (\textit{gTTS}) library.
\end{itemize}

\subsubsection{Conversational Agent \& RAG Pipeline}
The core intelligence is powered by \textbf{LangChain} and the \textbf{Google Gemini 2.5 Flash} model. The agent is configured with a persistent memory mechanism (\texttt{InMemorySaver}) to maintain context within a session. The system employs a decision-making strategy defined in its system prompt to select between three distinct tools:

\begin{enumerate}
    \item \textbf{Context Retrieval (RAG)}: 
    For queries regarding classical music or established knowledge, the agent utilizes a local \textbf{Chroma} vector database. Queries are embedded using \texttt{GoogleGenerativeAIEmbeddings} (model: \texttt{gemini-embedding-001}) to perform similarity searches against a pre-indexed collection of music-related Wikipedia documents.
    
    \item \textbf{Web Search}: 
    For current events or information not present in the local database, the agent employs \textit{DuckDuckGoSearchResults} to perform live web searches.
    
    \item \textbf{Web Loader}: 
    When specific external web pages are identified as satisfied sources, this tool utilizes \texttt{WebBaseLoader} to scrape and ingest the full textual content of the URL for synthesis.
\end{enumerate}

The agent logic prioritizes the local RAG tool for efficiency and reliability, falling back to web search tools only when necessary information is absent or requires real-time data.